...
export default function FavoritesScreen() {
  ...
  const favorites = useMemo(
    () => items.filter((item) => item.isFavorite).sort((a, b) => b.createdAt - a.createdAt),
    [items]
  );
    ...
    <View style={[styles.root, { paddingTop: topPad }]}>
      <View style={styles.header}>
        ...
        <View style={styles.headerActions}>
          <Pressable onPress={() => setViewMode(viewMode === 'grid' ? 'compact' : viewMode === 'compact' ? 'list' : 'grid')}>
            <Ionicons
              name={viewMode === 'list' ? 'list-outline' : viewMode === 'compact' ? 'apps-outline' : 'grid-outline'}
              size={20}
      ...
      {favorites.length === 0 ? (
        <View style={styles.empty}>
          ...
          <Text style={styles.emptySubtitle}>Tap the heart icon on any item to save it here</Text>
        </View>
      ) : (
        <FlatList
          key={viewMode}
          ...
          renderItem={({ item }) => (
            <View style={[styles.cardWrapper, viewMode !== 'list' && { flex: 1 }]}>
              <MediaCard
                ...
                tags={tags}
                viewMode={viewMode}
                onPlay={() => router.push({ pathname: '/player/[id]', params: { id: item.id } })}
